...
export function useFormValidation<T extends Record<string, any>>(initialValues: T) {
  ...
  const validateField = (name: string, value: string, otherValues?: Record<string, any>) => {
    ...
    switch (name) {
      ...
      case 'password':
        ...
        if (otherValues?.confirmPassword) {
          const confirmError = validators.validateConfirmPassword(otherValues.confirmPassword, value);
          setErrors(prev => ({ ...prev, confirmPassword: confirmError }));
        }
        break;
      case 'confirmPassword':
        error = validators.validateConfirmPassword(value, otherValues?.password || '');
        break;
      default:
        break;
    }
    ...
    fields.forEach(field => {
      ...
      switch (field) {
        ...
        case 'password':
          ...
          break;
        case 'confirmPassword':
          newErrors[field] = validators.validateConfirmPassword(value, formData.password as string);
          break;
        default:
          break;
      }
