    ...
    <div className="w-full h-fit flex flex-col items-start justify-between">
        <div className="relative">
            <Link href={`/products/details/${product.id}`} >
                <Image src={product.imagePath} alt={product.name} width={400} height={400} className="object-contain hover:scale-105 transition-all border border-gray-200 rounded-2xl shadow-sm hover:brightness-50"/>
                {product.isDiscounted && <span className="absolute rounded-full p-2 aspect-square text-center content-center border border-gray-200 shadow-sm bg-white font-bold text-primary right-4 top-4">-{product.discountPercent}%</span>}
            </Link>
        <h3 className="mt-3 font-bold text-2xl">{product.name}</h3>
        <p className="text-base text-gray-700 mt-2 line-clamp-2 h-16">{product.description}</p>
