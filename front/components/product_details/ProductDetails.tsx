...
const ProductDetails: React.FC<ProductDetailsComponentProps> = ({productId}) => {
  ...
  if (!product) {
    ...
    return (<div className="px-20 py-16 grid grid-cols-2 gap-8 items-center">
        <div><Image src={product.imagePath} alt={product.name} width={600} height={600} className="object-cover"/></div>
        <ProductInfo {...product}/>
    </div>);
}
