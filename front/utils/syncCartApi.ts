...
import { addMultipleItemsToDatabase, checkAuthStatus, getUserCartFromDatabase } from "./cartApi";
...
export const syncCartWithDatabase = async () => {
  ...
  if (userDBCart.length > 0 && userLSCart.length > 0) {
    const dbProductIds = new Set(userDBCart.map(item => item.productId.toString()));
    ...
    const mergedCart = userDBCart.map(dbItem => {
        const lsItem = userLSCart.find(
            lsItem => lsItem.productId.toString() === dbItem.productId.toString()
        );
    ...
    const uniqueLSItems = userLSCart.filter(
        lsItem => !mergedCart.some(
            mergedItem => mergedItem.productId.toString() === lsItem.productId.toString()
        )
    );
