...
import { getAllUsers, getUserById, createUser, deleteUser, getUserByEmail } from "../services/user.service";
...
export async function createUserHandler(request: FastifyRequest<{ Body: CreateUserBody }>, reply: FastifyReply) {
    try {
        ...
        const userCart = await createCart(request.server, {"userId": newUser.id});
