...
export async function getAllProducts(fastify: FastifyInstance) {
    return fastify.prisma.product.findMany({select: {
        id: true,
        name: true,
        imagePath: true,
        description: true,
...
export async function getProductsByPrice(fastify: FastifyInstance, { priceMin, priceMax }: { priceMin?: number, priceMax?: number }) {
    return fastify.prisma.product.findMany({
        where: {
